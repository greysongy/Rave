...
struct SelfPostView: View {
    ...
    var alert = AlertToast(displayMode: .alert, type: .regular, title: "DELETED")
    ...
    var body: some View {
        ...
        .actionSheet(isPresented: $showOptionsSheet) {
            ActionSheet(title: Text("Delete Post?"), message: nil, buttons: [
                .destructive(Text("Delete")) {
                    postManager.deletePost(post: self.post) { success in
                        if success {
    ...
    private func removePostFromList() {
        if let index = postManager.myPosts.firstIndex(where: { post in
            self.post.id == post.id
        }) {
            self.postManager.myPosts.remove(at: index)
        }
    }
}

struct SelfPostView_Previews: PreviewProvider {
    static var previews: some View {
        SelfPostView(postManager: PostManager(), commentsManager: CommentsManager(), post: Post(id: "asd"))
